...
class FirebaseWalkService : WalkService {
 ...
 private val WalkReference: DatabaseReference =
  FirebaseDatabase.getInstance().reference.child("Walks")

 override fun loadWalksFromDatabase(callback:
 ...
 override fun loadWalksOnceFromDatabase(callback:
                                        OnServiceDataCallback<List<Walk>>) {
  val walkListener = object : ValueEventListener {
   override fun onDataChange(datasnapshot: DataSnapshot) {
    val walklist: MutableList<Walk> = mutableListOf()
 ...
 override fun loadWalk(walkId: Int, callback:
 OnServiceDataCallback<Walk>) {
  val walkListener = object : ValueEventListener {
   override fun onDataChange(datasnapshot: DataSnapshot) {
  ...
  WalkReference.child(walkId.toString())
   .addValueEventListener(walkListener)
 }
