...
class FirebaseUserService : UserService {
 ...
 override fun loadUsersFromDatabase(callback:
 ...
 override fun loadUsersOnceFromDatabase(callback:
                                        OnServiceDataCallback<List<User>>) {
  val usersListener = object : ValueEventListener {
   override fun onDataChange(datasnapshot: DataSnapshot) {
    val userlist: MutableList<User> = mutableListOf()
 ...
 override fun loadUser(userId: String, callback:
 OnServiceDataCallback<User>) {
  val userListener = object : ValueEventListener {
   override fun onDataChange(datasnapshot: DataSnapshot) {
    if (datasnapshot.getValue(User::class.java) == null)
     usersReference.child(userId).removeEventListener(
     this
    )
    else {
 ...
 override fun loadUserOnce(userId: String, callback:
 OnServiceDataCallback<User>) {
  val userListener = object : ValueEventListener {
   override fun onDataChange(datasnapshot: DataSnapshot) {
    if (datasnapshot.getValue(User::class.java) == null)
